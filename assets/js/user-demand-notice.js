...
async function getAgentId(){
    try {
        const response = await fetch(`${basePath}/business-owner/${id}`, {
            method: "GET",
            headers: {
        ...
        const agent = result.data || []; // Ensure 'data' exists

        // typecast the agent ID to a number
        let aId = parseInt(agent.agent_id, 10) || null;
        if (isNaN(aId)) {
            aId = 0;
            console.error("Invalid agent ID:", agent.agent_id);
            // return;
        }
...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
        const response = await fetch(`${basePath}/business-owner/${id}/demand-notices`, {
            method: "GET",
            headers: {
                "Content-Type": "application/json",
            },
                    ...
                    if (verifyResult.status === 1 && verifyResult.message.length > 0) {
                        ...
                        if (paymentStatus === "paid" && externalNotice.invoice_number === notice.demand_notice_number) {
                            ...
                            if (success) {
                                ...
                                rowData[5] = "paid"; // Update the status column
